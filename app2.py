...
from prophet import Prophet  # Added for forecasting
...
holiday_sales = data[data['Date'].dt.month == 11]
...
st.subheader("📈 Time Series Forecasting (Next 12 Weeks)")

prophet_df = data[['Date', 'Weekly_Sales']].rename(columns={'Date': 'ds', 'Weekly_Sales': 'y'})
...
fig5 = px.line(forecast, x='ds', y='yhat', title="Sales Forecast for Next 12 Weeks")
fig5.add_scatter(x=prophet_df['ds'], y=prophet_df['y'], mode='lines', name='Actual Sales')
st.plotly_chart(fig5, use_container_width=True)

st.write("📅 Forecasted Sales Table (Next 12 Weeks)")
st.dataframe(forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(12).rename(
...
if 'prediction' in locals():
    ...
    pdf.multi_cell(0, 10, txt=(
        ...
        "Random Forest and XGBoost, time series forecasting with Prophet, and anomaly detection "
